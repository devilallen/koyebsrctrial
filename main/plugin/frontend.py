...
async def check_time_gap(user_id: int):
    ...
    if str(user_id) in GAP:
        ...
        if round(current_time - previous_time) < 300:
            return True, round(previous_time - current_time + 300)
        elif round(current_time - previous_time) >= 300:
            del GAP[str(user_id)]
            return False, None
...
async def clone(event):
    ...
    for li in li:
        ...
        if s == True:
            await event.reply(r)
            return
        is_in_gap, sleep_time = await check_time_gap(event.sender_id)
        if is_in_gap:
         await event.reply(f"Sorry Sir No Flooding Allowed Send New Link After `{str(sleep_time)}s` !!")
         return
